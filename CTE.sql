...
WITH CTE_EXAMPLE as
(
select gender as gen,avg(salary) as avg_sal,max(salary) as max_sal,min(salary) as min_sal from employeement_demographic dem join employeement_salary sal on dem.employee_id=sal.employee_id group by gender
)
